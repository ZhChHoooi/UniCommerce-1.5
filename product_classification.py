...
class ECInstructProcessor:
    def __init__(self, api_key: str, max_concurrent: int = 100, semaphore_limit: int = 50):
        ...
        self.single_task = "Multiclass_Product_Classification"
    ...
    def get_system_prompt(self) -> str:
        return (
            "As an expert in multi-type product classification, compare the user query with the product title to determine if the product meets the query specifications then select the option that best describes their relevance, and respond only with the selected option."
        )

    def get_few_shot_examples(self) -> str:
        Multiclass_Product_Classification_example = """
Multiclass_Product_Classification Example 1
task:Multiclass_Product_Classification
instruction:Determine the relevance between the query and the product title provided, and select your response from one of the available options.
...
<answer>A: The product is relevant to the query, and satisfies all the query specifications.</answer>

Multiclass_Product_Classification Example 2
task:Multiclass_Product_Classification
instruction:Determine the relevance between the query and the product title provided, and select your response from one of the available options.
input:{"query": "percussion accessory back", "product title": "MOICO Handheld Back Massager, 6 Interchangeable Nodes, 10 Speeds & 12 Modes, Cordless Massagers Handheld Deep Tissue Massager for Muscles, Neck, Shoulder, Arms, Leg, Calf, Foot, Full Body Pain Relief"}
Options: ["A: The product is relevant to the query, and satisfies all the query specifications.", "B: The product is somewhat relevant. It fails to fulfill some aspects of the query but the product can be used as a functional substitute.", "C: The product does not fulfill the query, but could be used in combination with a product exactly matching the query.", "D: The product is irrelevant to the query."]

Reasoning:
1. Query analysis: The query is "percussion accessory back." This suggests the user is looking for an accessory related to percussion (typically in music, such as drums) that is associated with "back."
2. Product examination: The product is a "Handheld Back Massager" with percussion-style massage functionality (deep tissue, interchangeable nodes, vibration).
3. Relevance check: The term "percussion" exists conceptually, as percussion massage devices are often described as using percussion technology. The product also directly relates to "back."
4. Gap analysis: However, the query could be interpreted as musical percussion equipment, not massage devices. Therefore, while it aligns with the "percussion" and "back" elements in a massage context, it may not fully satisfy a user looking for musical instrument accessories.
5. Conclusion: The product can be considered a functional substitute if the query intent was "percussion massage for back," but it fails if the query intent was music-related.
<answer>B: The product is somewhat relevant. It fails to fulfill some aspects of the query but the product can be used as a functional substitute.</answer>
"""
        return  Multiclass_Product_Classification_example
...
async def main():
    print("=" * 60)
    print("ECInstruct Dataset Processor - Single Task (Multiclass_Product_Classification) - Async")
    ...
    INPUT_FILE = "Multiclass_Product_Classification.json"
    OUTPUT_FILE = "Multiclass_Product_Classification_output.json"
    FAILED_IDS_FILE = "Multiclass_Product_Classification_failed_ids.json"
    FAILED_ITEMS_FILE = "Multiclass_Product_Classification_failed_items.json"
    MAX_CONCURRENT = 200
    SEMAPHORE_LIMIT = 100
    SAVE_INTERVAL = 1000
