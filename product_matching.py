...
class ECInstructProcessor:
    def __init__(self, api_key: str, max_concurrent: int = 100, semaphore_limit: int = 50):
        ...
        self.single_task = "Product_Matching"
    ...
    def get_system_prompt(self) -> str:
        return (
         "As an expert in product matching, determine accurately using product information whether the two products are identical, then respond only with yes if they are identical or no if they are not, based solely on the detailed product information provided."
        )

    def get_few_shot_examples(self) -> str:
        Product_Matching_example = """
Product_Matching Example 1
...
input:{"product 1": {"title": "media studio for psp standard editon", "description": "media studio for psp standard editon (win 98 me 2000 xp)", "manufacturer": "encore software", "price": "29.99"}, "product 2": {"title": "media studio for psp standard editon (800060)", "description": "media studio for psp standard editon brand: encore software mpn: 800060 variant name: media studio for psp standard editon", "manufacturer": "nan", "price": "47.99"}}
option:Null

Reasoning:
1. Title comparison: Both products have nearly identical titles ("media studio for psp standard editon"), with product 2 adding "(800060)" which appears to be an MPN reference.
2. Description comparison: Both descriptions refer to the same software product, with product 1 listing OS compatibility and product 2 listing brand and MPN details.
3. Manufacturer comparison: Product 1 lists Encore Software as manufacturer; product 2 shows "nan" but its description explicitly confirms Encore Software.
4. Price comparison: The price difference ($29.99 vs $47.99) can be attributed to retailer or market variation, not product difference.
5. Conclusion: Despite minor metadata differences, both represent the same software product — Media Studio for PSP Standard Edition by Encore Software.
<answer>yes</answer>

Product_Matching Example 2
task:Product_Matching
instruction:Analyze the title, description, manufacturer, and price between the two products below and generate an output of yes if the two products are the same, otherwise respond with no.
...
<answer>no</answer>
"""
        return Product_Matching_example
...
async def main():
    print("=" * 60)
    print("ECInstruct Dataset Processor - Single Task (Product_Matching) - Async")
    ...
    INPUT_FILE = "Product_Matching.json"
    OUTPUT_FILE = "Product_Matching_output.json"
    FAILED_IDS_FILE = "Product_Matching_failed_ids.json"
    FAILED_ITEMS_FILE = "Product_Matching_failed_items.json"
    MAX_CONCURRENT = 200
    SEMAPHORE_LIMIT = 100
    SAVE_INTERVAL = 1000
