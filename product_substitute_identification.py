...
class ECInstructProcessor:
    def __init__(self, api_key: str, max_concurrent: int = 100, semaphore_limit: int = 50):
        ...
        self.single_task = "Product_Substitute_Identification"
    ...
    def get_system_prompt(self) -> str:
        return (
             "As an expert in product substitute identification, determine whether the product is partially relevant to the query and can functionally substitute unmet requirements, using product and query information, then respond with yes if it can or no if it cannot."
        )

    def get_few_shot_examples(self) -> str:
        Product_Substitute_Identification_example = """
Product_Substitute_Identification Example 1
task:Product_Substitute_Identification
...
Product_Substitute_Identification Example 2
task:Product_Substitute_Identification
instruction:Given a query and a product, identify if the product is somewhat relevant to the query. It fails to fulfill some aspects of the query but the product can be used as a functional substitute. Only output yes or no.
input:{"query": "west highland terrier puzzle", "product": "SUNSOUT INC Pets in The Park 300 pc Jigsaw Puzzle"}

Reasoning:
1. Query intent: The user wants a puzzle specifically themed around a **West Highland Terrier (Westie)**.
2. Product examination: The given product is a **Pets in The Park** jigsaw puzzle with 300 pieces, which may include various pets or dogs.
3. Relevance check: While the puzzle is animal/pet-themed and could potentially feature dogs, it does not guarantee a dedicated **West Highland Terrier** focus.
4. Substitution logic: The product is somewhat relevant (it is a dog/pet puzzle) but fails to fully satisfy the specific breed requirement.
5. Conclusion: It can work as a **functional substitute**, though not an exact match.
<answer>yes</answer>
"""
        return Product_Substitute_Identification_example
...
async def main():
    print("=" * 60)
    print("ECInstruct Dataset Processor - Single Task (Product_Substitute_Identification) - Async")
    ...
    INPUT_FILE = "Product_Substitute_Identification.json"
    OUTPUT_FILE = "Product_Substitute_Identification_output.json"
    FAILED_IDS_FILE = "Product_Substitute_Identification_failed_ids.json"
    FAILED_ITEMS_FILE = "Product_Substitute_Identification_failed_items.json"
    MAX_CONCURRENT = 100
    SEMAPHORE_LIMIT = 50
    SAVE_INTERVAL = 200
